...
“逐条独立复核”展示每条候选的等待/运行状态、复核支持、复核排除、证据不足或执行失败，
附复核模型、明确结论与缺少证据。历史无复核记录的线索显示“未复核”，不会被新界面补标为已验证。
原报告可以跳转到同版本的“补充复核”，补跑页面也可返回原始扫描。
详见 [逐条候选复核与补跑](candidate-verification.md)。
