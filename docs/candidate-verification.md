...
从 0.6.0 开始，服务器每日复核队列先合并 Astra 与 Grok Build 两条独立初筛通道，再合并
各自的 `findings.json` 和 `coverage.json.deferred`。模型间一致只记录来源，不作为真实性证明。
后者必须含标题和实质问题描述/证据；纯覆盖备注、扫描中断、限流记录和安全拒绝不作为漏洞送审。
按候选稳定标识去重，保留反证、原始暂缓原因及代码位置。每条单独运行，按配置最多 8 条。

Codex 初筛使用 Astra / max，将全 VM 清单拆成 5 个顺序分片，每片最长运行 2 小时；
Grok Build 订阅初筛最长 3 小时；复核使用
Astra / xhigh，每条候选（含同模型重试）
共享 60 分钟窗口。配置中的 200/30/240 美元数值仅作为运维预算参考：当前 Codex Security
CLI 没有 Astra 成本模型，传入 `--max-cost` 会在发起模型请求前失败，因此运行清单会明确
标记美元上限未强制执行。
...
Astra 和 5.5 当前都没有可用的 CLI 成本计价支持，所以二者由运行时长、候选数和进度
看门狗约束，不宣称有美元硬上限。
